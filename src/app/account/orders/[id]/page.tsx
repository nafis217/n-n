    ...
    <div className="min-h-screen bg-[#FAFAFA] text-black py-10 md:py-16 px-4 sm:px-8 md:px-12">
      <div className="max-w-5xl mx-auto space-y-8">
        ...
        <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 pb-6 border-b border-neutral-200">
          <Link
            href="/account/orders"
            className="flex items-center gap-2 text-xs font-mono uppercase tracking-wider text-neutral-500 hover:text-black transition-colors"
          >
          ...
          <div className="flex items-center gap-3">
            {order.status !== 'CANCELLED' && order.status !== 'DELIVERED' && (
              <button
                ...
                className="px-4 py-2 border border-neutral-300 bg-white text-neutral-700 hover:border-black hover:text-black text-xs font-mono uppercase tracking-wider transition-colors"
              >
                Cancel Order
              </button>
            )}
            <button
              ...
              className="px-4 py-2 border border-neutral-300 hover:border-black text-xs font-mono uppercase tracking-wider text-black flex items-center gap-2 transition-colors bg-white"
            >
          ...
          <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
            ...
            {order.trackingNumber && (
              <div className="text-right font-mono text-xs">
                <span className="text-neutral-500 block text-[10px] uppercase">Courier Waybill:</span>
                <span className="text-black font-bold">{order.trackingNumber}</span>
              </div>
            )}
          </div>
        </div>
        ...
        <div className="p-6 sm:p-8 bg-white border border-neutral-200 shadow-sm space-y-6">
          <h2 className="font-display text-sm uppercase tracking-widest font-bold text-black flex items-center gap-2 pb-4 border-b border-neutral-200">
            <Truck className="w-4 h-4 text-black" />
            <span>Fulfillment &amp; Courier Transit Journey</span>
          </h2>

          <div className="space-y-6 relative before:absolute before:inset-0 before:left-4 before:w-0.5 before:bg-neutral-200">
            {order.timeline.map((event, idx) => (
              <div key={idx} className="relative flex items-start gap-4 pl-2">
                <div
                  className={`w-5 h-5 rounded-full border-2 flex items-center justify-center shrink-0 z-10 ${
                    event.isCompleted
                      ...
                      : 'bg-neutral-100 border-neutral-300 text-transparent'
                  }`}
                >
                ...
                <div className="flex-1 min-w-0 pb-2">
                  <div className="flex flex-col sm:flex-row justify-between items-baseline gap-1">
                    <h3
                      className={`font-display text-xs uppercase tracking-wider font-bold ${
                        event.isCompleted ? 'text-black' : 'text-neutral-400'
                      }`}
                    >
                      {event.title}
                    </h3>
                  ...
                  <p className="text-xs text-neutral-600 font-sans mt-0.5 leading-relaxed">
                    {event.description}
                  </p>
                </div>
              </div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-8">
          ...
          <div className="lg:col-span-7 p-6 bg-white border border-neutral-200 shadow-sm space-y-4">
            <h3 className="font-display text-xs uppercase tracking-widest font-bold text-black pb-3 border-b border-neutral-200">
                  ...
                  <div className="flex items-center gap-4">
                    ...
                    <div>
                      <Link
                        ...
                        className="font-display text-xs uppercase font-bold text-black hover:underline transition-colors"
                      >
                        {item.title}
                      </Link>
          ...
          <div className="lg:col-span-5 space-y-6">
            {/* Address */}
            <div className="p-6 bg-white border border-neutral-200 shadow-sm space-y-2 text-xs font-mono">
              <span className="text-neutral-500 uppercase tracking-wider block text-[10px] font-bold">
                Recipient Details:
              </span>
            ...
            {/* Financial Breakdown */}
            <div className="p-6 bg-white border border-neutral-200 shadow-sm space-y-3 text-xs font-mono text-neutral-600">
              <div className="flex justify-between">
                <span>Subtotal</span>
                <span className="text-black font-semibold">৳{order.subtotal.toLocaleString()}</span>
              </div>
              {order.discount > 0 && (
      ...
      {cancelModalOpen && (
        <div className="fixed inset-0 z-[9999] flex items-center justify-center p-4">
          ...
          />
          <div className="relative w-full max-w-md bg-white border border-neutral-300 text-black p-6 sm:p-8 shadow-2xl z-10 space-y-4">
            ...
            <input
              ...
              className="w-full bg-neutral-50 border border-neutral-300 px-3.5 py-2.5 text-xs text-black placeholder-neutral-400 focus:outline-none focus:border-black focus:bg-white"
            />
            <div className="flex gap-3 pt-2">
              ...
              <button
                ...
                className="flex-1 py-3 bg-black hover:bg-neutral-800 text-white text-xs font-display uppercase tracking-widest font-bold"
              >
                Confirm Cancel
              </button>
            </div>
